...
export default class Dropdown extends PureComponent {
  ...
  onPress(event) {
    ...
    this.container.measureInWindow((x, y, containerWidth, containerHeight) => {
        ...
        if (null == dropdownPosition) {
          ...
          if (~selected) {
            if (dropdownPosition < 0) {
              offset =
                this.itemSize() *
                (selected - visibleItemCount - dropdownPosition);
            } else {
      ...
      let top =
        y + Platform.select({ ios: 1, android: 2 }) + labelHeight - itemPadding;
  ...
  selectedIndex() {
    ...
    return data.findIndex(({ value }) => value === this.state.value);
  }

  selectedItem() {
    let { data = [] } = this.props;

    return data.find(({ value }) => value === this.state.value);
  }
  ...
  renderBase() {
    ...
    let title = "string" === typeof label ? label : value;
  ...
  renderItems() {
    ...
    return data.map(({ value, label = value }, index) => {
      let color = ~selected
        ? index === selected ? selectedItemColor : itemColor
        : selectedItemColor;
